...
consequences {
    accepted (
        ...
        "**AMENDED (+2). One module joins the surface: `script`, a reading of a script without running it.** Ronin composes a recursive `$(MAKE)` out of a recipe line, and to do that soundly it has to know what the line would do — which commands, in what order, under what condition, and whether each piece of each word is quoted — which is a question only the shell that will run the line can answer without disagreeing with itself. So `nsh::script::Reader` parses and does not execute, and hands back a PROJECTION: `Command`, `Word`, `Piece`, all `#[non_exhaustive]`, saying `Other` and `Expansion` where they do not describe a form. The tree in `crate::nodes` stays `pub(crate)`, with everything this decision closed. The count moves by one module and its handful of types; the property — a surface two orders of magnitude below the internals, measured under `missing_docs` — holds. `docs/api-design.md` §2.2."
