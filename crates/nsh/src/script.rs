...
use crate::word::{ParameterOperation, ParsedWord, QuoteBoundary, WordPart};
        ...
        crate::resource::with_resources(shell, |shell, _resources| {
            ...
            loop {
                match crate::parser::parse_command(shell, false)? {
                    ...
                    ParseResult::Tree(Some(node)) => commands.push(command(&node)),
...
#[non_exhaustive]
pub struct Word {
    /// The word's pieces, in order. Empty for `''` and `""`.
...
fn command(node: &Node) -> Command {
    match node {
        Node::Command(simple) => Command::Simple(simple_command(simple)),
        Node::Pipeline(pipeline) => {
            let commands = Command::Pipeline {
                negated: false,
                commands: pipeline.commands.iter().map(command).collect(),
            };
        ...
        Node::Redirect(compound) => redirected(command(&compound.command), &compound.redirections),
        Node::Background(compound) => Command::Background(Box::new(redirected(
            command(&compound.command),
            &compound.redirections,
        ))),
        Node::Subshell(compound) => redirected(
            Command::Subshell(Box::new(command(&compound.command))),
            &compound.redirections,
        ),
        Node::And(binary) => Command::And(
            Box::new(command(&binary.left)),
            Box::new(command(&binary.right)),
        ),
        ...
        Node::If(conditional) => Command::If {
            condition: Box::new(command(&conditional.condition)),
            then: Box::new(command(&conditional.then_branch)),
            otherwise: conditional
                .else_branch
                .as_deref()
                .map(|branch| Box::new(command(branch))),
        },
        Node::While(binary) => Command::While {
            condition: Box::new(command(&binary.left)),
            body: Box::new(command(&binary.right)),
        },
        ...
        Node::For(iteration) => Command::For {
            ...
            words: iteration.words.iter().filter_map(word_of).collect(),
            body: Box::new(command(&iteration.body)),
        },
        Node::Case(selection) => Command::Case {
            word: word_of(&selection.word).unwrap_or_else(empty_word),
            clauses: selection
                .clauses
                .iter()
                .map(|clause| CaseClause {
                    patterns: clause.patterns.iter().filter_map(word_of).collect(),
                    body: clause.body.as_deref().map(|body| Box::new(command(body))),
        ...
        Node::Function(definition) => Command::Function {
            ...
            body: Box::new(command(&definition.body)),
        },
        Node::Not(negated) => {
            let (negated, commands) = match command(&negated.command) {
...
fn redirected(command: Command, redirections: &[nodes::Redirection]) -> Command {
    if redirections.is_empty() {
        return command;
    }
    Command::Redirected {
        command: Box::new(command),
        redirections: redirections.iter().map(redirection).collect(),
    }
}

fn redirection(redirection: &nodes::Redirection) -> Redirection {
    Redirection {
        source: nodes::source::redirection(redirection),
    }
}

fn simple_command(simple: &nodes::SimpleCommand) -> SimpleCommand {
    SimpleCommand {
        assignments: simple
            .assignments
            .iter()
            .filter_map(|node| match node {
                Node::Word(word) => Some(assignment(&word.word)),
                _ => None,
            })
            .collect(),
        words: simple.arguments.iter().filter_map(word_of).collect(),
        redirections: simple.redirections.iter().map(redirection).collect(),
    }
}
...
fn word_of(node: &Node) -> Option<Word> {
    match node {
        Node::Word(word) => Some(word_from_parts(word.word.parts(), &word.word)),
        _ => None,
    }
}
...
/// first part holds the whole name.
fn assignment(word: &ParsedWord) -> Assignment {
    let parts = word.parts();
    let Some((WordPart::Literal(first), rest)) = parts.split_first() else {
        return Assignment {
            name: BString::default(),
            value: word_from_parts(parts, word),
        };
    };
    let at = first
        .iter()
    ...
    // The value is the rest of the first part, followed by the rest of the
    // parts. Rendered from a word rebuilt that way so its `source` is the
    // value's own spelling.
    let mut value_parts = Vec::with_capacity(rest.len() + 1);
    if !value_head.is_empty() {
        value_parts.push(WordPart::Literal(BString::from(value_head)));
    }
    value_parts.extend(rest.iter().cloned());
    let value = ParsedWord::from_parts(value_parts);
    Assignment {
        name,
        value: word_from_parts(value.parts(), &value),
    }
}

fn word_from_parts(parts: &[WordPart], word: &ParsedWord) -> Word {
    let mut pieces: Vec<Piece> = Vec::new();
    let mut quoted = false;
    let push_literal = |pieces: &mut Vec<Piece>, bytes: &[u8], quoted: bool| {
        if let Some(Piece::Literal {
            bytes: previous,
            quoted: previous_quoted,
        }) = pieces.last_mut()
    ...
    for part in parts {
        match part {
            WordPart::Literal(bytes) => push_literal(&mut pieces, bytes, quoted),
            WordPart::Escaped(byte) => push_literal(&mut pieces, &[*byte], true),
            WordPart::Multibyte { bytes, escaped } => {
                push_literal(&mut pieces, bytes, quoted || *escaped);
            }
            WordPart::Quote(QuoteBoundary::Open) => quoted = true,
            WordPart::Quote(QuoteBoundary::Close) => quoted = false,
            WordPart::Parameter(parameter) => {
                let plain = parameter.operation == ParameterOperation::Value
                    && !parameter.indirect
                    && !parameter.colon
                    && parameter.operand.is_none();
                pieces.push(if plain {
                    Piece::Parameter {
                        name: parameter.name.clone(),
                        quoted,
                    }
                } else {
                    Piece::Expansion { quoted }
                });
            }
            WordPart::Command(_) => pieces.push(Piece::CommandSubstitution),
            WordPart::Arithmetic(_) => pieces.push(Piece::Arithmetic),
        }
    }
    Word {
        pieces,
        source: nodes::source::word(word),
    }
}
