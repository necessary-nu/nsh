...
//!   * `crate::variables::{bltinlookup, histsizeval}`
//!   * `crate::number::parse_decimal`
//!   * `crate::evaluation::evalstring`
...
pub(crate) fn history_mut(shell: &mut crate::context::Shell) -> Option<&mut History> {
    shell.editor.history.as_mut()
}

#[must_use]
pub fn history_active(shell: &crate::context::Shell) -> bool {
    shell.editor.history.is_some()
}

#[must_use]
pub fn editing_active(shell: &crate::context::Shell) -> bool {
    shell.editor.editor.is_some()
}
...
pub fn read_edit_line(
    ...
    let Some(mut editor) = shell.editor.editor.take() else {
        return Ok(0);
    };
    let Some(mut history) = shell.editor.history.take() else {
        shell.editor.editor = Some(editor);
        return Ok(0);
    };
    ...
    let result = editor.read_into(shell, &mut history, destination);
    shell.editor.history = Some(history);
    shell.editor.editor = Some(editor);
    result
}
...
pub fn record_history_line(
    ...
    let recorded = {
        let Some(history) = history_mut(shell) else {
            return;
        };
        if first {
    ...
    if !recorded {
        ...
        shell.editor.history = None;
        shell.editor.history_file = None;
        return;
    }
    save_history(shell);
}
...
pub fn refresh_editor_configuration(shell: &mut crate::context::Shell) {
    if shell.options.enabled(ShellOption::Interactive) {
        if !history_active(shell) {
            crate::error::with_interrupts_deferred(shell, |shell| {
                shell.editor.history = Some(History::new());
                shell.editor.history_file =
                    crate::variables::lookup_bytes(shell, BStr::new(b"HISTFILE"))
            ...
            set_history_size(shell, BStr::new(size.as_slice()));
            let mut saved = Vec::new();
            let file_ready = match shell.editor.history_file.as_mut() {
                Some(file) => {
                    file.rewind().is_ok()
        ...
        let stdin = shell.descriptors.get(LogicalDescriptor::STDIN);
        let stderr = shell.descriptors.get(LogicalDescriptor::STDERR);
        let mode = if shell.options.enabled(ShellOption::Vi) {
            Some(EditingMode::Vi)
        } else if shell.options.enabled(ShellOption::Emacs)
        ...
        } else if mode.is_none() && editing_active(shell) {
            crate::error::with_interrupts_deferred(shell, |shell| {
                shell.editor.editor = None;
            });
        }

        if let (Some(mode), Some(editor)) = (mode, shell.editor.editor.as_mut()) {
...
pub(crate) fn save_history(shell: &mut crate::context::Shell) {
    if shell.shell_level != 0 {
        return;
    }
    let Some(history) = shell.editor.history.as_ref() else {
        return;
    };
    let contents = history.file_contents();
    let Some(file) = shell.editor.history_file.as_mut() else {
        return;
    };
...
pub fn set_history_size(shell: &mut crate::context::Shell, hs: &BStr) {
    let histsize = if hs.is_empty() {
        ...
        let mut input = hs
            .iter()
            .copied()
            .skip_while(|byte| shell.locale.is_space(*byte));
...
mod tests {
    ...
    fn noninteractive_ignores_editor_request() {
        ...
        refresh_editor_configuration(&mut shell);
    ...
    fn unset_histsize_retains_posix_minimum() {
        ...
        shell.editor.history = Some(History::new());
        set_history_size(&mut shell, BStr::new(b""));

        let history = shell.editor.history.as_mut().unwrap();
