//! Reading a number out of the bytes it was written as.
//!
//! `printf.c` called `strtoimax`, `strtoumax` and `strtod` and then read
//! `ep` and `errno`; the port has none of those, so the conversions are
//! written out here, and each returns the same three answers -- the
//! value, where the conversion stopped, and whether the magnitude ran
//! past what the type holds. [`super::Operands::check_conversion`] turns
//! the last two into the warning and the exit status. Base 0's radix
//! detection and `strtod`'s longest-valid-prefix rule are the parts
//! Rust's own `parse` will not do, and both are the specification rather
//! than an implementation detail of it.
//!
//! Half the file is C's hexadecimal floating literal, whose digits *are*
//! the value's bits: [`scan_hexadecimal`] lays them into a mantissa and a
//! binary exponent so that [`round_to_double`] can round the whole thing
//! exactly once. Rounding a mantissa and then scaling it by a power of
//! two rounds twice, and the power underflows to zero well before the
//! value does.
//!
//! [`leading_number`] is the one here that reads the *format* string
//! rather than an operand. It is the same act -- bytes to a number,
//! saturating instead of failing -- but the format walk that calls it
//! stays in [`super`].
...
pub(super) fn scan_double(locale: &nsh_platform::Locale, bytes: &[u8]) -> (f64, usize, bool) {
...
pub(super) fn leading_number(bytes: &[u8]) -> usize {
