...
use crate::nodes::{
    ...
    BashArrayValue, BashAssignmentOperator, BashFunction, BashFunctionStyle, BashNode,
    BashProcessDirection, BashProcessSubstitution, Node, NodeText, SourceTokens, WordNode,
};
...
/// `[[ ]]`, whose sub-grammar is a recursive descent of its own rather
/// than a production of the one here. It reaches back for `take_word` and
/// `ConditionalWord`, which `compound_array` below shares with it.
mod conditional;
